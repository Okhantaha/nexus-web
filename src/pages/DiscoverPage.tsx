...
export default function DiscoverPage() {
  ...
  const humanQuery = useQuery({
    ...
    queryFn: () => api.get<VenueSuggestion[]>('/api/lunch/oneriler').then((r) => r.data),
  })

  const venues = tab === 'ai' ? (aiQuery.data ?? []) : (humanQuery.data ?? [])
            ...
            <AnimatePresence>
              {remaining.slice(0, 3).map((venue, i) => (
                <VenueCard
                  ...
                  isTop={i === 0}
                  suggesterName={tab === 'human' ? (venue as VenueSuggestion).oneren ?? 'Kullanıcı' : undefined}
                  onSwipe={handleSwipe}
                />
              ))}
            </AnimatePresence>
      ...
      {remaining.length > 0 && !loading && (
        <div className="px-5 pb-24 flex flex-col items-center gap-4">
          <div className="flex items-center gap-6">
            ...
            <motion.button whileTap={{ scale: 0.9 }} onClick={() => handleSwipe()}
              className="w-14 h-14 rounded-full bg-white shadow-card flex items-center justify-center border border-green-100">
